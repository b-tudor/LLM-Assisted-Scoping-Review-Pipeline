...
if (argc != 2):
    ...
    print(f'JSONL data is output to terminal. Redirect output to a JSONL file.\nE.g.:')
    print(f'\t{sys.argv[0]} input_file.csv > output_file.jsonl')
    print(f"\tThis expects an input CSV file with columns 'id', 'decision', 'confidence', and 'reason'")
    print(f'\tAlthough, practically speaking, it only checks for id and decision.')
    exit(1)
...
# Load the CSV file
N=0 # A count of articles in the file, in case we need it
...
for id in df['id']:
    ...
    try:
        ...
        with open(filename, 'r', encoding='utf-8') as file:
            content = file.read()
            file.close()
            # Strip the outer quotes for processing, which will include some quote manipulation:
...
print("id,text")
for id, contents in zip(df['id'], df['text']):

    ID = int(id)
    # Here is the aforementioned quote processing (the way json escapes these won't fly in a CSV file)
    # I think we are converting something like \"  ->  \"" , which will collapse to \" once read in as
    # a CSV, which will collapse to " when imported into a json object.
